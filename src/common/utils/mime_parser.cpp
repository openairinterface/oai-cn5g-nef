...
bool mime_parser::parse(const std::string& str) {
  std::string CRLF = "\r\n";
  Logger::nef_app().debug("Parsing the message with Simple Parser");
  ...
  Logger::nef_app().debug("Boundary: %s", boundary_str.c_str());
  ...
  while (boundary_pos < boundary_last_post) {
    ...
    Logger::nef_app().debug("Content Type: %s", p.content_type.c_str());
    ...
    if (boundary_pos > 0) {
      ...
      Logger::nef_app().debug("Body: %s", p.body.c_str());
...
unsigned char* mime_parser::format_string_as_hex(const std::string& str) {
  ...
  Logger::nef_app().debug(
      ...
      str.c_str());
  Logger::nef_app().debug("Data (formatted):");
#if DEBUG_IS_ON
