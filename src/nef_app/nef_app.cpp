...
void nef_app::handle_analytics_fetch(
  ...
  // Return locally cached analytics from existing subscriptions.
...
void nef_app::handle_qos_subscription_patch(
  ...
  // This resource is a T8 AsSessionWithQoSSubscription, not the PCF
  // AppSessionContext. There is no typed *Patch model,
  // so apply an RFC 7396 JSON merge-patch to the stored body and
...
// shape.
//
// The entry method runs on the dispatcher worker. It does the same
// pre-southbound work its synchronous ancestor did — authorize, parse and
// validate, SSRF-check, update the local store — bookending that with
// set_request_bearer_token() and clear_request_bearer_token(), and answering
// through the sink on any early return. Then it copies whatever the
...
// already known. It holds no bearer token and re-locks every store it
// touches, since the world may have moved on since the entry method ran. It
// applies the handler's failure policy and completes the deferred response
// with the same status and body the synchronous version would have produced.
// ═══════════════════════════════════════════════════════════════════════════

//------------------------------------------------------------------------------
// The pre-southbound work is unchanged from the sync handler; the AMF
// subscribe becomes an async fire, and the rollback and post-wiring move into
...
//------------------------------------------------------------------------------
// pfd_app_put — a single best-effort call: the not-found/forbidden/parse block
// + local store are unchanged; the UDR PUT becomes an async
// fire; the response is built from the locally-stored app json regardless of
// the UDR result (best-effort — UDR failure is warn-only).
...
//------------------------------------------------------------------------------
// traffic_influence_update: pre-southbound block
// (authorize/parse/validate/SSRF + owner lookup + pcf_policy_id resolve)
// unchanged; PCF PATCH becomes an async fire; the failure
// branch and the post-commit session overwrite + 200
// move into cont_ti_update.
...
void nef_app::cont_ti_update(
  ...
  // A PCF failure fails the whole request with 502.
  if (!sbi_ok(r)) {
    Logger::nef_app().warn(
...
//------------------------------------------------------------------------------
// traffic_influence_patch: pre-southbound block
// (authorize/typed-patch-validate + owner lookup + merge_patch into a local
// patched_copy + pcf_policy_id resolve) unchanged; PCF PATCH becomes an
// async fire; failure and post-commit session overwrite + 200 move into
// cont_ti_patch.
...
//------------------------------------------------------------------------------
// qos_subscription_create: the pre-southbound block
// The pre-southbound work is unchanged; the PCF create becomes an async
// fire. The 500 failure branch, the wiring, and the success body with its
// relative `self` URI all move into cont_qos_create. Rewriting that `self`
// into an absolute URI stays in the adapter header sink.
//
...
// sync create_pcf_policy_auth) and then applies the
// same is_valid_app_session_id() guard the sync handler applies. For a
// southbound timeout/0/4xx/5xx the parsed id is empty/invalid → 500.
...
void nef_app::cont_qos_create(
  ...
  // 500 rather than 502 here: PCF has to both succeed and hand back a
  // usable appSessionId.
  ...
  // A concurrent delete may have removed qos_sub_id while PCF
...
void nef_app::cont_ti_create_pcf(
  ...
  const std::string pcf_policy_id =
      sbi_ok(r) ? nef_async_parse_pcf_app_session_id(r) : "";
  // 502 and roll back only our own state: PCF never committed anything.
  ...
  // A concurrent delete may have removed ti_id while PCF was in
  ...
  // while holding a store mutex) and uses the discovery-free *_at_async
  // variant.
...
//------------------------------------------------------------------------------
// monitoring_event_subscription_delete: the
// authorize/owner block is unchanged; the AMF unsubscribe,
// result unchecked) becomes an async fire; local cleanup + 204
// run regardless of the southbound outcome (best-effort, sets 204
// unconditionally).
void nef_app::monitoring_event_unsubscribe(
  ...
  // Fire the AMF unsubscribe. If there is no NF sub id
  // there is nothing to unsubscribe — finish the cleanup inline (the sync path
  // skips the southbound call).
  if (nf_sub_id.empty()) {
...
//------------------------------------------------------------------------------
// qos_subscription_update (PUT): authorize/typed-parse/
// validate/owner/immutability/app-session block + local store update are
// unchanged; the PCF policy-auth update becomes an async fire. Best-effort:
// the AF gets the stored subscription data whatever PCF says, and a PCF
// failure only warns.
//
// When the PCF body cannot be translated the sync path sends nothing at all,
// so the async path matches it by finishing inline.
...
// qos_subscription_patch: authorize/owner/merge-
// patch/validate/SSRF/immutability/app-session block + local store update are
// unchanged; the PCF update becomes an async fire; the
// response is the patched body regardless of the PCF outcome
// (best-effort, PCF failure warn-only). When the PCF body cannot be
// translated the sync path fires no call— finish inline.
...
// qos_subscription_delete: authorize/owner block
// unchanged; the SMF unsubscribe (result unchecked) becomes an
// async fire; local cleanup + 204 run regardless (best-effort).
...
void nef_app::cont_qos_delete(
  ...
  if (!nf_sub_id.empty()) {
    const std::lock_guard<std::shared_mutex> lock(m_nf2af_mutex);
    m_nf2af_sub_id.erase(qos_sub_id);  // keyed by notifId == qos_sub_id
  }
  remove_subscription(qos_sub_id);
  release_af_profile_subscription(af_id, qos_sub_id);
  nef_audit::log(
...
//------------------------------------------------------------------------------
// bdt_policy_create: authorize/typed-parse/
// validate/bdtPolData/path-param block + bdt_id generation + local store
// unchanged; the PCF BDT create becomes an async
// fire. SUCCESS-ON-3xx: a 303 is success (the sync `!= SEE_OTHER` guard,
// genuine failure → local rollback + 502
// On success the 201 body is built in the continuation. the
// sync handler does not parse the PCF id into the AF response — it only stores
// pcf_bdt_id in m_bdt_id2pcf_policy_id. The async cont extracts that id
// via nef_async_parse_pcf_bdt_policy_id, which mirrors the sync
// create_pcf_bdt_policy precedence exactly: Location
...
void nef_app::cont_bdt_create(
  ...
  // A concurrent delete may have removed bdt_id while PCF was
...
//------------------------------------------------------------------------------
// bdt_policy_update: the authorize / parse / validate / owner-check /
// pcf-id-resolve block is unchanged; the PCF BDT update becomes an async
// fire. A failure fails the request with 502; on success the local store
// overwrite and the 200 both run in the continuation.
...
//------------------------------------------------------------------------------
// bdt_policy_patch: the authorize / owner-check / pcf-id-resolve work and
// the merge-patch into a local copy are unchanged; the PCF BDT update
// becomes an async fire. A failure fails the request with 502; on success
// the re-parse, validate, store and 200 all run in the continuation.
...
//------------------------------------------------------------------------------
// bdt_policy_delete: authorize/owner/pcf-id-resolve
// unchanged; the PCF BDT delete becomes an async fire; the local erase +
// 204 run regardless (best-effort).
...
//------------------------------------------------------------------------------
// pfd_create: authorize/pfdDatas/path-param block
// unchanged; the UDR PFD PUT becomes an
// async fire; the 201 echo body is returned regardless of the UDR
// outcome (best-effort).
...
//------------------------------------------------------------------------------
// pfd_delete: authorize unchanged; the UDR
// delete (result unchecked) becomes an async fire; 204
...
//------------------------------------------------------------------------------
// pfd_get: authorize is unchanged; the UDR PFD GET becomes an async fire.
// This one propagates the upstream status rather than flattening it: a UDR
// 404 becomes an AF 404 saying "PFD data not found", 2xx becomes 200, and
// anything else becomes 502.
//
// That is why it does not go through the generic helper, which would
// collapse the 404 into a 502. handle_pfd_get is currently unrouted (no
...
void nef_app::cont_pfd_get(
  ...
  // Forward the upstream UDR status as-is, rather than the
  // helper). The sync handle_pfd_get checks STRICT 200, not any
...
//------------------------------------------------------------------------------
// pfd_app_patch: authorize/owner/app-lookup +
// merge-patch + typed re-parse + local store unchanged; the
// The UDR PFD PUT becomes an async fire, best-effort: its result is only
// warned about, and the 200 echo body goes back either way.
...
//------------------------------------------------------------------------------
// pfd_app_delete: authorize/owner/app-lookup +
// local erase unchanged; the UDR delete (result
// unchecked) becomes an async fire; 204 regardless (best-effort).
...
//------------------------------------------------------------------------------
// nnef_pfd_put_app: authorize/normalize + local
// transaction store unchanged; the UDR PFD PUT becomes an async fire; the
// 201/200 echo body + PFD-subscriber notification run regardless of the UDR
// outcome (best-effort).
...
//------------------------------------------------------------------------------
// nnef_pfd_delete_app: authorize/transaction-lookup
// + local erase unchanged; the UDR delete (result
// warn-only) becomes an async fire; the PFD-subscriber notify + 204
// run regardless of the UDR outcome (best-effort).
...
// Each dispatcher worker resolves the UDR endpoint
...
// discovery-free *_at_async leg and advances, rolls back or finishes. No
// thread is ever parked, and discover_nf never runs on oai-http-io, which is
// what would deadlock the io pool against itself.
...
// pfd_transaction_delete — N-loop UDR DELETE, best-effort 204
void nef_app::pfd_transaction_delete(
  ...
  // resolve UDR HERE (dispatcher worker), thread by value.
  ...
  // Audit the 204 now — it is already decided — then kick the delete
...
void nef_app::pfd_transaction_delete_step(std::shared_ptr<PfdDeleteChain> st) {
  ...
  m_nef_client->udr_delete_pfd_data_at_async(
      st->udr_ep, app_id, [this, st, app_id](oai::sba::response r) mutable {
        ...
        // matching the sync per-app delete whose result is unchecked.
        if (!sbi_ok(r)) {
          Logger::nef_app().warn(
...
// span async hops. in the previous version, SNAPSHOTS the iteration set under
// the lock (applying the requested_ids filter, copying {app_id,
// fallback}=stored app_data per RELEASES the lock, then runs the read cursor.
// Each step fires udr_get_pfd_data_at_async; the continuation pushes (sbi_ok ?
// parsed(r) : fallback) with ["applicationId"]=app_id; the final step sends 200
// with the array. No nef_app lock is held across any async hop.
void nef_app::nnef_pfd_partial_pull(
  ...
  if (!m_nef_client->discover_nf(nf_type_t::NF_TYPE_UDR, st->udr_ep)) {
    // No UDR to refresh from, so serve every entry out of the stored
    // app_data instead, exactly as the sync path does when the read fails.
    Logger::nef_app().warn(
...
void nef_app::nnef_pfd_partial_pull_step(std::shared_ptr<PfdPullChain> st) {
  ...
  m_nef_client->udr_get_pfd_data_at_async(
      ...
      [this, st, app_id, fallback](oai::sba::response r) mutable {
        // Best-effort per app, matching the sync path:
        ...
        // it strictly compares == OK, not 2xx). Reproduce
        // that exactly.
        nlohmann::json entry;
        if (r.status_code == http_status_code::OK) {
...
//------------------------------------------------------------------------------
// pfd_transaction_put
// authorize/validate/typed-parse of pfdDatas + create-vs-update
...
// continuation (mirrors the PfdRollbackTracker). On full success
...
void nef_app::pfd_transaction_put(
  ...
  // Determine create-vs-update before UDR writes
  {
    std::shared_lock lock(m_pfd_mutex);
    st->is_create =
        (m_pfd_trans_sessions.find(trans_id) == m_pfd_trans_sessions.end());
  ...
  if (!m_nef_client->discover_nf(nf_type_t::NF_TYPE_UDR, st->udr_ep)) {
    // No UDR write happened yet, so there is nothing to roll back. 500 (the
    // sync path aborts the transaction with 500 on a UDR write failure).
    Logger::nef_app().error(
...
void nef_app::pfd_put_rollback(
  ...
  // No local state was committed yet, so rollback is southbound
  // only.
...
//------------------------------------------------------------------------------
// traffic_influence_delete
// authorize/not-found/owner checks, captures the PCF policy id,
// and resolves PCF+UDR endpoints. cont_ti_delete_pcf fires the PCF app-session
// DELETE (only when a PCF policy exists), then cont_ti_delete_udr fires
// the UDR influence DELETE; both failures are warn-only and the
// response is always 204. Local state is erased in the FINAL continuation
// (after both SBI calls), preserving the sync ordering.
...
void nef_app::cont_ti_delete_pcf(
  ...
  // (best-effort, warn-only) and go straight to the final step.
  if (udr_ep.empty()) {
    Logger::nef_app().warn(
...
void nef_app::cont_ti_delete_udr(
    ...
    response_sink sink) {
  // Best-effort: UDR failure is warn-only and does not change the 204.
  // r.status_code==0 here means the UDR leg was skipped (already
  ...
  // Recover the PCF policy id under the lock to clean
  // m_nf2af_sub_id.
  std::string pcf_policy_id;
  {
    const std::lock_guard<std::shared_mutex> lock(m_ti_mutex);
...
void nef_app::nnef_pfd_put_transaction(
  ...
  // resolve UDR HERE (dispatcher worker).
  if (!m_nef_client->discover_nf(nf_type_t::NF_TYPE_UDR, st->udr_ep)) {
...
// PUT each app; on failure → (nnef_put_rollback, 500);
// when all apps PUT → nnef_put_after_commit.
...
// Compensating DELETEs over committed[0..k-1] in REVERSE → 500
// (fatal: 500, mirrors PfdRollbackTracker ). Southbound only — no
...
// Commit local state, SEND the success response.
void nef_app::nnef_put_after_commit(std::shared_ptr<NnefPutChain> st) {
  ...
  // SEND THE SUCCESS RESPONSE NOW, After this move the sink is spent
  // (exactly-once);
  ...
  // POST-COMMIT best-effort removed-app cleanup. Fire-
  // and-forget async DELETEs; failures are warn-only and ignored; nothing is
  // rolled back and the response has already gone out. The continuations
  // capture value copies only (tid,
