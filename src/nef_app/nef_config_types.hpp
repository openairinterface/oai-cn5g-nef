...
class nef_config_type : public oai::config::nf {
 ...
 public:
  ...
  /**
   * Returns true when insecure_dev_mode is explicitly enabled in config.
   * When true the NEF will allow all requests even if no JWT secret and no
   * AF whitelist are configured (fail-open).  Default: false (fail-closed).
   */
