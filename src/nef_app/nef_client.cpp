...
// Helpers for NRF registration and discovery — build URIs, parse responses, etc.
...
static std::string nf_type_to_str(nf_type_t t) {
  switch (t) {
    case NF_TYPE_AMF:  return "AMF";
    case NF_TYPE_SMF:  return "SMF";
    case NF_TYPE_PCF:  return "PCF";
    case NF_TYPE_UDR:  return "UDR";
    case NF_TYPE_UDM:  return "UDM";
    case NF_TYPE_NRF:  return "NRF";
    case NF_TYPE_AUSF: return "AUSF";
...
bool nef_client::register_to_nrf() {
  ...
  Logger::nef_app().info("Registering NEF to NRF (instance: %s)...",
                         m_nef_instance_id.c_str());
  ...
  auto add_service = [&](const std::string& svc_name,
    ...
    svc["versions"]          = nlohmann::json::array(
        {nlohmann::json{{"apiVersionInUri", version},
...
bool nef_client::send_heartbeat_to_nrf() {
  ...
  // PATCH /nf-instances/<id>  with [{"op":"replace","path":"/nfStatus","value":"REGISTERED"}]
...
bool nef_client::discover_nf(nf_type_t nf_type, std::string& nf_endpoint) {
  ...
  std::string cfg_key;
  switch (nf_type) {
    case NF_TYPE_AMF: cfg_key = AMF_CONFIG_NAME; break;
    case NF_TYPE_SMF: cfg_key = SMF_CONFIG_NAME; break;
    case NF_TYPE_PCF: cfg_key = PCF_CONFIG_NAME; break;
    case NF_TYPE_UDR: cfg_key = UDR_CONFIG_NAME; break;
    default:          cfg_key = "";
  }
    ...
    try {
      ...
      if (nf_cfg) {
        ...
        std::string scheme =
            "http";
        nf_endpoint = scheme + "://" +
                      std::string(inet_ntoa(sbi.get_addr4())) + ":" +
                      std::to_string(sbi.get_port());
        Logger::nef_app().debug(
            "NF %s endpoint from config: %s",
            cfg_key.c_str(), nf_endpoint.c_str());
        return true;
      }
    } catch (...) {}
  }
  ...
  // Cache check — if we have a cached endpoint for this NF type, use it without querying NRF
  {
    std::string cached_ep;
    if (nrf_discovery_cache::instance().get(nf_type_str, cached_ep)) {
      Logger::nef_app().debug(
          "NF discovery cache hit: %s → %s",
          nf_type_str.c_str(), cached_ep.c_str());
  ...
  if (last_disc_resp.status_code != http_status_code::OK) {
    Logger::nef_app().warn(
        "NF discovery for %s failed (status %d)",
        nf_type_str.c_str(), last_disc_resp.status_code);
    return false;
  }
  ...
  try {
    ...
    if (inst.contains("nfServices") && !inst["nfServices"].empty()) {
      ...
      nf_endpoint = scheme + "://" +
                    ep.value("ipv4Address", "") + ":" +
                    std::to_string(ep.value("port", 8080));
    } else if (inst.contains("ipv4Addresses") &&
               !inst["ipv4Addresses"].empty()) {
      nf_endpoint = "http://" +
                    inst["ipv4Addresses"][0].get<std::string>() + ":8080";
    } else {
      return false;
    }
    ...
    Logger::nef_app().warn(
        "NF discovery parse error: %s", e.what());
    return false;
  }
}
...
bool nef_client::subscribe_amf_event_exposure(
  ...
  if (!discover_nf(nf_type_t::NF_TYPE_AMF, amf_url)) {
    Logger::nef_app().warn("AMF not found — cannot subscribe to event exposure");
    return false;
  }
  std::string url = amf_url + nef_sbi_helper::AmfEventExposureBase + "v1/subscriptions";

  // Inject NEF's own callback URL so AMF knows where to send event notifications.
  // We use a placeholder sub-id here; after creation we update the NF→AF mapping.
  // TS 29.518: field is "notificationDestination"
  nlohmann::json sub_body = subscription_data;
  const std::string nef_callback_base =
      get_nef_notify_uri("__placeholder__");
...
bool nef_client::unsubscribe_amf_event_exposure(
  ...
  auto resp = http_client_inst->send_http_request(
      oai::common::sbi::method_e::DELETE, req);
  return (resp.status_code == http_status_code::NO_CONTENT ||
...
bool nef_client::subscribe_smf_event_exposure(
  ...
  if (!discover_nf(nf_type_t::NF_TYPE_SMF, smf_url)) {
    Logger::nef_app().warn("SMF not found — cannot subscribe to event exposure");
    return false;
  }
  std::string url = smf_url + nef_sbi_helper::SmfEventExposureBase + "v1/subscriptions";
...
bool nef_client::udr_put_pfd_data(
  ...
  int status = sbi_call_with_retry(
      ...
      sbi_circuit_breaker_registry::instance(),
      sbi_sleep_udr, sbi_log_udr);
  return (status == http_status_code::OK || status == http_status_code::CREATED);
}
...
bool nef_client::forward_notification_to_af(
  ...
  return retry_with_backoff(
      ...
      sleep_fn,
      /*max_attempts=*/3,
      circuit_breaker_registry::instance());
}
