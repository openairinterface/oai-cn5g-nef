...
bool nef_client::subscribe_amf_event_exposure(
  ...
  Logger::nef_app().warn(
      "AMF event subscription failed (status %d)", resp.status_code);
  return false;
}
...
bool nef_client::subscribe_smf_event_exposure(
  ...
  Logger::nef_app().warn(
      "SMF event subscription failed (status %d)", resp.status_code);
  return false;
}
...
bool nef_client::create_pcf_policy_auth(
  ...
  if (resp.status_code == http_status_code::CREATED ||
    ...
    if (app_session_id.empty()) {
      std::string location =
          get_header_case_insensitive(resp.headers, "Location");
      app_session_id = extract_last_path_segment(location);
    }

    return is_2xx_status(resp.status_code) && !app_session_id.empty();
  }
  return false;
}
