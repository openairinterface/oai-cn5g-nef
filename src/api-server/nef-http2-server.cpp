...
//------------------------------------------------------------------------------
// Pull the token out of an "Bearer <token>" Authorization header, matching
...
// worker's http2_response, then block until it fires. Returns false, having
// already sent a 503, if the dispatcher refused the work — the caller should
// just return.
...
//------------------------------------------------------------------------------
// Same, for delete-style handlers: the sink delivers an empty body, which is
// forwarded as-is with no content-type and whatever headers the caller gave.
template<typename DispatchFn>
...
//
// header_fn sees the status and the parsed body, and the body is mutable on
// purpose: this is where a relative `self` gets rewritten to an absolute URI.
// It is re-serialized afterwards.
...
void nef_http2_server::start() {
  ...
  //
  // The size is configurable via dispatcher_pool_size. The DEFAULT
  // (override == 0) stays http_workers + 2 — byte-identical to prior behavior.
  // NOTE: the dispatcher pool is the de-facto concurrency
...
void nef_http2_server::stop() {
  // With async dispatch the default mode, teardown must stop HTTP intake BEFORE
  // draining the dispatcher so no new work_items / southbound fires arrive
  // while the dispatcher drains:
  //
  ...
  //      the event loop exits (http2_server.cpp pool_.reset()), joined via
...
void nef_http2_server::handle_monitoring_event_subscribe(
  ...
  // handle and return immediately — no fut.wait().
