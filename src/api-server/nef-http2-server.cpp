...
class request_auth_scope {
 public:
  request_auth_scope(nef_app* nef, const request& req) : m_nef(nef) {
    if (m_nef) {
...
void nef_http2_server::handle_ti_get(
    const std::string& af_id, const std::string& ti_id, const response& res) {
  nlohmann::json resp_body;
  int http_code = 0;
  m_nef_app->handle_traffic_influence_get(
      af_id, ti_id, resp_body, http_code, 2);
  header_map headers;
...
void nef_http2_server::handle_ti_list(
    const std::string& af_id, const response& res) {
  nlohmann::json resp_body;
  int http_code = 0;
...
void nef_http2_server::handle_monitoring_event_update(
  ...
  try {
    ...
    end_http2_error(
        res, 400, "Bad Request", "Missing or invalid request payload");
    return;
  }
  nlohmann::json resp_body;
  int http_code = 0;
  m_nef_app->handle_monitoring_event_subscription_update(
      scs_as_id, sub_id, json_body, resp_body, http_code, 2);
  header_map headers;
  headers.insert({"content-type", {"application/json", false}});
  res.write_head(http_code, headers);
  res.end(resp_body.dump());
...
void nef_http2_server::start() {
  Logger::nef_sbi().info(
      "NEF HTTP/2 server listening on %s:%d", m_address.c_str(), m_port);

  // ── Nnef_EventExposure
  // /nnef-eventexposure/v1/subscriptions[/{subscriptionId}]
  m_server.handle(
      "/nnef-eventexposure/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
          if (len != 0) return;
          request_auth_scope auth_scope(m_nef_app, req);
          ...
          if (pfx_pos == std::string::npos) {
            end_http2_error(
                res, 404, "Not Found", "Requested resource was not found");
            return;
          }
  ...
  m_server.handle(
      "/3gpp-monitoring-event/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
  ...
  m_server.handle(
      "/3gpp-traffic-influence/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
  ...
  // ── BDT PATCH /3gpp-bdt/v1/{afId}/bdtPolicies/{bdtPolicyId}
  m_server.handle("/3gpp-bdt/", [&](const request& req, const response& res) {
    std::string acc;
    req.on_data([&req, &res, &acc, this](const uint8_t* data, std::size_t len) {
      ...
      if (bdt_path == "bdtPolicies" && req.method() == "PATCH" &&
          !bdt_id.empty()) {
        handle_bdt_patch(af_id, bdt_id, acc, res);
      } else {
        // fallback to existing BDT handlers
        if (req.method() == "POST") {
          handle_bdt_create(af_id, acc, res);
        } else if (req.method() == "GET" && bdt_id.empty()) {
          handle_bdt_get(af_id, "", res);  // list
        } else if (req.method() == "GET" && !bdt_id.empty()) {
          handle_bdt_get(af_id, bdt_id, res);
        } else if (req.method() == "PUT" && !bdt_id.empty()) {
          handle_bdt_update(af_id, bdt_id, acc, res);
        } else if (req.method() == "DELETE" && !bdt_id.empty()) {
          handle_bdt_delete(af_id, bdt_id, res);
        } else {
          end_http2_error(
              res, 405, "Method Not Allowed",
              "HTTP method is not supported for this resource");
  ...
  m_server.handle(
      "/3gpp-pfd-management/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
          ...
          auto scs_as_id =
              (s1 != std::string::npos) ? rest.substr(0, s1) : rest;
          auto after_scs = (s1 != std::string::npos) ? rest.substr(s1 + 1) : "";
          // after_scs = "transactions" | "transactions/{transId}" |
  ...
  m_server.handle(
      "/nnef-pfdmanagement/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
  ...
  m_server.handle("/3gpp-bdt/", [&](const request& req, const response& res) {
    ...
    req.on_data([&req, &res, &acc, this](const uint8_t* data, std::size_t len) {
      ...
      if (req.method() == "PATCH" && policy_path == "bdtPolicies" &&
        ...
        handle_bdt_get(af_id, pol_id, res);
      } else if (req.method() == "PUT" && !pol_id.empty()) {
        handle_bdt_update(af_id, pol_id, acc, res);
      } else if (req.method() == "DELETE" && !pol_id.empty()) {
        handle_bdt_delete(af_id, pol_id, res);
      } else {
        end_http2_error(
            res, 405, "Method Not Allowed",
            "HTTP method is not supported for this resource");
  ...
  // /3gpp-monitoring-event/v1/{scsAsId}/subscriptions/{subId} (Extend existing
  // handler) Already handled in the main /3gpp-monitoring-event/ route, add PUT
  // support: (No code needed here, just ensure handler is implemented below) ──
  // Analytics /fetch endpoint ──
  m_server.handle(
      "/3gpp-analyticsexposure/", [&](const request& req, const response& res) {
        std::string acc;
        req.on_data([&req, &res, &acc, this](
                        const uint8_t* data, std::size_t len) {
          acc.append(reinterpret_cast<const char*>(data), len);
  ...
  m_server.handle("/nef-notify/", [&](const request& req, const response& res) {
    std::string accumulated;
    req.on_data(
        [&req, &res, &accumulated, this](const uint8_t* data, std::size_t len) {
  ...
  if (m_server.listen_and_serve(ec, m_address, std::to_string(m_port), true)) {
    Logger::nef_sbi().error("HTTP/2 server error: %s", ec.message().c_str());
    return;
  }
  m_running = true;
...
void nef_http2_server::handle_pfd_create(
    const std::string& app_id, const std::string& body, const response& res) {
  nlohmann::json json_body = {};
  try {
    json_body = nlohmann::json::parse(body);
  } catch (...) {
...
void nef_http2_server::handle_bdt_delete(
    const std::string& af_id, const std::string& bdt_id, const response& res) {
  int http_code = 0;
...
void nef_http2_server::handle_qos_delete(
    const std::string& af_id, const std::string& sub_id, const response& res) {
  int http_code = 0;
...
void nef_http2_server::handle_nnef_pfd_list_transactions(const response& res) {
  nlohmann::json resp_body;
  int http_code = 0;
...
void nef_http2_server::handle_nnef_pfd_put_transaction(
    const std::string& trans_id, const std::string& body, const response& res) {
  nlohmann::json json_body = {};
  try {
    json_body = nlohmann::json::parse(body);
  } catch (...) {
...
void nef_http2_server::handle_nnef_pfd_get_transaction(
    const std::string& trans_id, const response& res) {
  nlohmann::json resp_body;
  int http_code = 0;
