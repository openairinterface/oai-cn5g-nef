...
const std::string AFLevelUELocationUpdateNotificationOperationApi::base =
    "/3gpp-mo-lcs-notify/v1";
...
void AFLevelUELocationUpdateNotificationOperationApi::setupRoutes() {
  ...
  Routes::Post(
      *router, base + "/",
      Routes::bind(
          &AFLevelUELocationUpdateNotificationOperationApi::root_post_handler,
          this));
...
AFLevelUELocationUpdateNotificationOperationApi::handleParsingException(
...
std::pair<Pistache::Http::Code, std::string>
AFLevelUELocationUpdateNotificationOperationApi::handleOperationException(
    const std::exception& ex) const noexcept {
  return std::make_pair(Pistache::Http::Code::Internal_Server_Error, ex.what());
}

void AFLevelUELocationUpdateNotificationOperationApi::root_post_handler(
  ...
  try {
    ...
    try {
      nlohmann::json::parse(request.body()).get_to(locUpdateData);
      locUpdateData.validate();
    } catch (std::exception& e) {
    ...
    try {
      this->root_post(locUpdateData, response);
...
void AFLevelUELocationUpdateNotificationOperationApi::
    af_level_ue_location_update_notification_operation_api_default_handler(
