...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_nef.html', 'w')
		self.generateHeader()
	...
	def generateHeader(self):
		...
		for reportFile in glob.glob('./*results_oai_*.html'):
			if reportFile == './test_results_oai_nef.html':
				continue
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI NEF <font color="Gold">Ubuntu18</font> Image</th>\n')
		self.file.write('		<th>OAI NEF <font color="Gold">RHEL8</font> Image</th>\n')
		self.file.write('     </tr>\n')
	...
	def initialGitSetup(self):
		...
		self.analyze_docker_build_git_part('NEF')
		self.file.write('	 </tr>\n')
	...
	def analyze_docker_build_git_part(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				section_end_pattern = 'WORKDIR /openair-nef/build/scripts'
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'build_nef --install-deps --force'
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search('FROM oai-nef-base', line)
						if result is not None:
							base_image = True
						result = re.search(section_start_pattern, line)
						if result is not None:
						...
						if result is not None:
							section_status = False
						if section_status:
							result = re.search('NEF deps installation successful', line)
							if result is not None:
								status = True
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_nef --install-deps --force\n'
				if base_image:
	...
	def buildCompileRows(self):
		...
		self.analyze_build_log('NEF', True)
		self.file.write('	 </tr>\n')
		self.file.write('	 <tr>\n')
		self.analyze_compile_log('NEF', True)
		self.file.write('	 </tr>\n')
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if section_status:
							...
							if result is not None:
								correctLine = re.sub("^.*/openair-nef","/openair-nef",line.strip())
	...
	def copyConfToolsToTargetImage(self):
		...
		self.analyze_copy_conf_tool_log('NEF')
		self.file.write('	 </tr>\n')
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('NEF', True)
		self.file.write('	 </tr>\n')
...
def Usage():
	...
	print('   Generate an HTML report for the Jenkins pipeline on openair-nef.')
	print('----------------------------------------------------------------------------------------------------------------------')
